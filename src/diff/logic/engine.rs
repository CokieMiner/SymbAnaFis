...
impl Expr {
    ...
    fn derive_impl(&self, var: &str, var_id: u64, ctx: &Context) -> Self {
        match &self.kind {
            ...
            ExprKind::FunctionCall { name, args } => {
                Self::derive_function_call(name, args, var, var_id, ctx)
            }
            ExprKind::Sum(terms) => Self::derive_sum(terms, var, var_id, ctx),
            ExprKind::Product(factors) => self.derive_product(factors, var, var_id, ctx),
            ExprKind::Div(u, v) => Self::derive_div(u, v, var, var_id, ctx),
            ExprKind::Pow(u, v) => Self::derive_pow(u, v, var, var_id, ctx),
            ExprKind::Derivative {
                inner,
                var: deriv_var,
                order,
            } => Self::derive_derivative(inner, deriv_var, *order, var, var_id),
            ExprKind::Poly(poly) => poly.derivative_expr(var),
        }
    }

    fn derive_function_call(
        name: &InternedSymbol,
        args: &[Arc<Self>],
        var: &str,
        var_id: u64,
    ...
    fn derive_sum(terms: &[Arc<Self>], var: &str, var_id: u64, ctx: &Context) -> Self {
    ...
    fn derive_product(&self, factors: &[Arc<Self>], var: &str, var_id: u64, ctx: &Context) -> Self {
        if factors.is_empty() {
            return Self::number(0.0);
        }
        if factors.len() == 1 {
    ...
    fn derive_div(u: &Self, v: &Self, var: &str, var_id: u64, ctx: &Context) -> Self {
        if let ExprKind::Number(n) = &u.kind {
            ...
            let f_squared = Self::pow_from_arcs(Arc::new(v.clone()), Arc::new(Self::number(2.0)));
            return Self::div_expr(neg_n_fprime, f_squared);
        }

        if let ExprKind::Number(_) = &v.kind {
            let u_prime = u.derive_impl(var, var_id, ctx);
            return Self::div_from_arcs(Arc::new(u_prime), Arc::new(v.clone()));
        }
        ...
        if u_is_zero && v_is_zero {
            ...
            Self::div_from_arcs(Arc::new(u_prime), Arc::new(v.clone()))
        } else if u_is_zero {
            let u_times_vprime = Self::mul_from_arcs(vec![Arc::new(u.clone()), Arc::new(v_prime)]);
            let neg_u_vprime = u_times_vprime.negate();
            let v_squared = Self::pow_from_arcs(Arc::new(v.clone()), Arc::new(Self::number(2.0)));
            Self::div_expr(neg_u_vprime, v_squared)
        } else {
            let u_prime_v = Self::mul_from_arcs(vec![Arc::new(u_prime), Arc::new(v.clone())]);
            let u_v_prime = Self::mul_from_arcs(vec![Arc::new(u.clone()), Arc::new(v_prime)]);
    ...
    fn derive_pow(u: &Self, v: &Self, var: &str, var_id: u64, ctx: &Context) -> Self {
        ...
        if !u_contains_var && !v_contains_var {
            ...
            if u_prime.is_zero_num() {
                ...
                let u_pow_n_minus_1 = Self::pow_from_arcs(Arc::new(u.clone()), Arc::new(n_minus_1));
            ...
            if v_prime.is_zero_num() {
                ...
                let a_pow_v = Self::pow_from_arcs(Arc::new(u.clone()), Arc::new(v.clone()));
            ...
            let term2 = if u_prime.is_zero_num() {
                ...
                let u_prime_over_u = Self::div_from_arcs(Arc::new(u_prime), Arc::new(u.clone()));
                Self::mul_from_arcs(vec![Arc::new(v.clone()), Arc::new(u_prime_over_u)])
            };
            ...
            if sum.is_zero_num() {
                ...
                let u_pow_v = Self::pow_from_arcs(Arc::new(u.clone()), Arc::new(v.clone()));
                if sum.is_one_num() {
                    u_pow_v
                } else {
    ...
    fn derive_derivative(
        inner: &Arc<Self>,
        deriv_var: &InternedSymbol,
        order: u32,
        var: &str,
        var_id: u64,
    ) -> Self {
