...
pub fn eval_hermite<T: MathScalar>(n: i32, x: T) -> T {
    if n < 0 {
        return T::nan();
    }
    if n == 0 {
...
pub fn eval_assoc_legendre<T: MathScalar>(l: i32, m: i32, x: T) -> T {
    if l < 0 || m.abs() > l {
        return T::nan();
    }
    let x_abs = x.abs();
...
pub fn eval_spherical_harmonic<T: MathScalar>(l: i32, m: i32, theta: T, phi: T) -> T {
    if l < 0 || m.abs() > l {
        return T::nan();
    }
