...
pub fn bessel_j<T: MathScalar>(n: i32, x: T) -> T {
    let n_abs = n.abs();
    let ax = x.abs();
    ...
    if ax < threshold {
        return if n_abs == 0 { T::one() } else { T::zero() };
    }
...
fn bessel_j_forward<T: MathScalar>(n: i32, x: T) -> T {
    let n_abs = n.abs();
    ...
    if n < 0 && n_abs % 2 == 1 { -jc } else { jc }
}

/// Miller's backward recurrence algorithm for `J_n(x)` when n > |x|
fn bessel_j_miller<T: MathScalar>(n: i32, x: T) -> T {
    let n_abs = n.abs();
    let two = T::from(2.0).expect("Failed to convert mathematical constant");
...
///
/// Reference: A&S §9.1.27, DLMF §10.6 <https://dlmf.nist.gov/10.6>
pub fn bessel_y<T: MathScalar>(n: i32, x: T) -> T {
    if x <= T::zero() {
        return T::nan();
    }
    let n_abs = n.abs();
...
pub fn bessel_k<T: MathScalar>(n: i32, x: T) -> T {
    if x <= T::zero() {
        return T::nan();
    }
    let n_abs = n.abs();
