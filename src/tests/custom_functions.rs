...
fn test_compiled_evaluation_with_context() {
    ...
    let evaluator =
        crate::VmEvaluator::compile(&expr, &["x"], Some(&ctx)).expect("Compilation should succeed");
...
fn test_compiled_evaluation_missing_context() {
    ...
    let result = crate::VmEvaluator::compile(&expr, &["x"], None);
...
fn test_arity_check() {
    ...
    let evaluator = crate::VmEvaluator::compile(&expr_ok, &["x", "y"], Some(&ctx))
    ...
    let result = crate::VmEvaluator::compile(&expr_bad, &["x"], Some(&ctx));
