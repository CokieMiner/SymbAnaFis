    ...
    |expr: &Expr, _context: &RuleContext| {
        if let AstKind::FunctionCall { name, args } = &expr.kind {
            if name == "sinh" && args.len() == 1 {
                if matches!(args[0].kind, AstKind::Number(n) if n == 0.0) {
        ...
        if let AstKind::Sum(terms) = &expr.kind {
            if terms.len() == 2 {
                ...
                if let Some((name1, arg1)) = get_hyperbolic_power(u, 2.0) {
                    if name1 == "cosh" {
                        if let Some(negated) = extract_negated(v) {
                            if let Some((name2, arg2)) = get_hyperbolic_power(&negated, 2.0) {
                ...
                if let AstKind::Number(n) = &u.kind {
                    if *n == 1.0 {
                        if let Some(negated) = extract_negated(v) {
                            if let Some((name, arg)) = get_hyperbolic_power(&negated, 2.0) {
                                if name == "tanh" {
                ...
                // coth^2(x) - 1 = csch^2(x)
                if let (Some((n1, a1)), AstKind::Number(num)) =
                    (get_hyperbolic_power(u, 2.0), &v.kind)
                {
                    if n1 == "coth" && (*num + 1.0).abs() < 1e-10 {
                        return Some(Expr::pow(Expr::func("csch", a1), Expr::number(2.0)));
                    }
                }
                if let (AstKind::Number(num), Some((n2, a2))) =
                    (&u.kind, get_hyperbolic_power(v, 2.0))
                {
                    if n2 == "coth" && (*num + 1.0).abs() < 1e-10 {
...
fn get_hyperbolic_power(expr: &Expr, power: f64) -> Option<(&str, Expr)> {
    if let AstKind::Pow(base, exp) = &expr.kind {
        if let AstKind::Number(p) = &exp.kind {
            if *p == power {
...
fn is_cosh_minus_sinh_term(expr: &Expr) -> Option<Expr> {
    ...
    if let AstKind::Sum(terms) = &expr.kind {
        if terms.len() == 2 {
            if let AstKind::FunctionCall { name: n1, args: a1 } = &terms[0].kind {
                if n1 == "cosh" && a1.len() == 1 {
                    if let AstKind::Product(factors) = &terms[1].kind {
                        if factors.len() == 2 {
                            if let AstKind::Number(n) = &factors[0].kind {
                                if *n == -1.0 {
                                    if let AstKind::FunctionCall { name: n2, args: a2 } =
                                        &factors[1].kind
                                    {
...
fn is_cosh_plus_sinh_term(expr: &Expr) -> Option<Expr> {
    ...
    if let AstKind::Sum(terms) = &expr.kind {
        if terms.len() == 2 {
            if let AstKind::FunctionCall { name: n1, args: a1 } = &terms[0].kind {
                if n1 == "cosh" && a1.len() == 1 {
                    if let AstKind::FunctionCall { name: n2, args: a2 } = &terms[1].kind {
                        if n2 == "sinh" && a2.len() == 1 && a1[0] == a2[0] {
                            return Some(a1[0].clone());
                        }
                    }
        ...
        if let AstKind::Sum(terms) = &expr.kind {
            if terms.len() == 2 {
                ...
                if let Some((c1, arg1, p1)) = parse_fn_term(u, "cosh") {
                    if let Some((c2, arg2, p2)) = parse_fn_term(v, "cosh") {
                        if arg1 == arg2 {
                            let eps = 1e-10;
...
fn parse_fn_term(expr: &Expr, func_name: &str) -> Option<(f64, Expr, f64)> {
    ...
    if let AstKind::Product(factors) = &expr.kind {
        if factors.len() == 2 {
            if let AstKind::Number(c) = &factors[0].kind {
                // c * func(x)
