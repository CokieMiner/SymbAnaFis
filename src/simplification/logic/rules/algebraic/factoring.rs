...
impl Rule for PerfectSquareRule {
    ...
    fn apply(&self, expr: &Arc<Expr>, _context: &RuleContext) -> Option<Arc<Expr>> {
        fn apply_inner(expr: &Arc<Expr>) -> Option<Arc<Expr>> {
                ...
                match &term.kind {
                    ...
                    AstKind::Product(_) => {
                        ...
                        if factors.len() == 1 {
                            ...
                            linear_terms.push((coeff, Arc::clone(&factors[0]), arc_number(1.0)));
            ...
            // Case 1: Standard perfect square a^2 + 2*ab + b^2
                    ...
                    && (sqrt_c2 - sqrt_c2.round()).abs() < EPSILON
                {
                    ...
                    if (expected_cross_abs - cross_coeff_abs).abs() < EPSILON && match_direct {
                        ...
                        let inner = if sign > 0.0 {
                            ...
                            let neg_b = Expr::product_from_arcs(vec![arc_number(-1.0), term_b]);
