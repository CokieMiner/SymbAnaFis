...
fn generate_mixed_complex(n: usize) -> String {
    ...
    for i in 1..=n {
        ...
        match i % 5 {
            0 => {
                write!(s, "{}*x^{}", i, i % 10 + 1).expect("write to String");
            }
            1 => {
                write!(s, "sin({i}*x)*cos(x)").expect("write to String");
            }
            2 => {
                write!(s, "(exp(x/{i}) + sqrt(x + {i}))").expect("write to String");
            }
            3 => {
                write!(s, "(x^2 + {i})/(x + {i})").expect("write to String");
            }
            4 => {
                write!(s, "sin(exp(x) + {i})").expect("write to String");
            }
            _ => panic!("i % 5 should always be in 0..=4"),
        }
    }
    s
}
...
#[test]
fn test_wrong_builtin_arity_is_rejected() {
    let x = crate::symb("x").to_expr();
    let y = crate::symb("y").to_expr();
    ...
    assert!(matches!(
        one_arg_result,
        Err(DiffError::UnsupportedFunction(_))
    ));
    assert!(matches!(
        two_arg_result,
        Err(DiffError::UnsupportedFunction(_))
...
fn test_eval_batch_missing_columns_default_to_zero_and_extra_ignored() {
    ...
    let z_vals = vec![11.0, 13.0, 17.0];
    let extra_vals = vec![101.0, 103.0, 107.0];

    let mut output = vec![0.0; x_vals.len()];
    eval.eval_batch(&[&x_vals], &mut output, None)
        .expect("Missing columns should default to zero");
    assert_eq!(output, vec![0.0, 0.0, 0.0]);

    eval.eval_batch(&[&x_vals, &y_vals, &z_vals, &extra_vals], &mut output, None)
...
#[test]
fn test_product_with_recip_sqrt_zero_remains_nan() {
    ...
    assert!(
        result.is_nan(),
        "x * 1/sqrt(x) at x=0 should be NaN, got {result}"
    );
}
...
fn test_sqrt_recip_negative_zero_remains_nan() {
    ...
    assert!(
        result.is_nan(),
        "sqrt(1 / -0.0) should be NaN, got {result}"
    );
}
...
#[test]
fn test_large_mixed_derivative_compile_regression() {
    let expr_s = generate_mixed_complex(100);
    let expr = parse_expr(&expr_s);
    let x = crate::symb("x");
    let deriv = crate::Diff::new()
