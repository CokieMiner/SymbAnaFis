...
fn main() {
    ...
    let current = diff(voltage.to_string(), "t".to_string(), Some(&["V0".to_string(), "R".to_string(), "C".to_string()]), None).unwrap();
    ...
    let heat_flux = diff(temperature.to_string(), "x".to_string(), Some(&["T0".to_string(), "alpha".to_string(), "t".to_string()]), None).unwrap();
    ...
    let prob_derivative = diff(probability_density.clone(), "x".to_string(), Some(&["sigma".to_string()]), None).unwrap();
    ...
    let shear_stress = diff(velocity.to_string(), "r".to_string(), Some(&["deltaP".to_string(), "mu".to_string(), "L".to_string(), "R".to_string()]), None).unwrap();
    ...
    let mag_sensitivity = diff(magnification.to_string(), "do".to_string(), Some(&["di".to_string()]), None).unwrap();
    ...
    let denom_derivative = diff(denominator.to_string(), "s".to_string(), Some(&["zeta".to_string(), "omega_n".to_string()]), None).unwrap();
    ...
    let pdf_derivative = diff(normal_pdf.to_string(), "x".to_string(), Some(&["mu".to_string(), "sigma".to_string()]), None).unwrap();
    ...
    let reaction_rate = diff(concentration.to_string(), "t".to_string(), Some(&["A0".to_string(), "k".to_string()]), None).unwrap();
    ...
    let particle_velocity = diff(pressure.to_string(), "x".to_string(), Some(&["p0".to_string(), "k".to_string(), "omega".to_string(), "t".to_string()]), None).unwrap();
