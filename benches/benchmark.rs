...
use symb_anafis::{diff, simplify, parse, simplify_ast};
...
fn bench_differentiation(c: &mut Criterion) {
    ...
    group.bench_function("poly_x^3+2x^2+x", |b| {
        b.iter(|| diff(black_box("x^3 + 2*x^2 + x".to_string()), "x".to_string(), None, None))
    });
    
    // Trigonometric
    group.bench_function("trig_sin(x)*cos(x)", |b| {
        b.iter(|| diff(black_box("sin(x) * cos(x)".to_string()), "x".to_string(), None, None))
    });
    
    // Chain rule
    group.bench_function("chain_sin(x^2)", |b| {
        b.iter(|| diff(black_box("sin(x^2)".to_string()), "x".to_string(), None, None))
    });
    
    // Exponential
    group.bench_function("exp_e^(x^2)", |b| {
        b.iter(|| diff(black_box("exp(x^2)".to_string()), "x".to_string(), None, None))
    });
    
    // Complex expression
    group.bench_function("complex_x^2*sin(x)*exp(x)", |b| {
        b.iter(|| diff(black_box("x^2 * sin(x) * exp(x)".to_string()), "x".to_string(), None, None))
    });
    
    // Quotient rule
    group.bench_function("quotient_(x^2+1)/(x-1)", |b| {
        b.iter(|| diff(black_box("(x^2 + 1) / (x - 1)".to_string()), "x".to_string(), None, None))
    });
    
    // Nested functions
    group.bench_function("nested_sin(cos(tan(x)))", |b| {
        b.iter(|| diff(black_box("sin(cos(tan(x)))".to_string()), "x".to_string(), None, None))
    });
...
fn bench_simplification(c: &mut Criterion) {
    ...
    group.bench_function("frac_add_(x^2+1)/(x^2-1)+1/(x+1)", |b| {
        b.iter(|| simplify(black_box("(x^2 + 1)/(x^2 - 1) + 1/(x + 1)".to_string()), None, None))
    });
...
fn bench_combined(c: &mut Criterion) {
    ...
    group.bench_function("d/dx[sin(x)^2]_simplified", |b| {
        b.iter(|| {
            let d = diff(black_box("sin(x)^2".to_string()), "x".to_string(), None, None).unwrap();
    ...
    group.bench_function("d/dx[(x^2+1)/(x-1)]_simplified", |b| {
        b.iter(|| {
            let d = diff(black_box("(x^2 + 1) / (x - 1)".to_string()), "x".to_string(), None, None).unwrap();
            simplify(d, None, None)
        })
    });
...
criterion_group!(benches, bench_parsing, bench_diff_ast, bench_simplify_ast, bench_differentiation, bench_simplification, bench_combined);
