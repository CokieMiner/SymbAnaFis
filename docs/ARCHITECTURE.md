...
flowchart TB
    ...
    subgraph "Bytecode Compiler"
        ...
        C2["CSE: Common Subexpression Elimination
        (Structural hashing + Cache)"]
        C3["Instruction Selection
        (Lowering AST to bytecode)"]
        C4["Register Allocation
        (Minimize stack usage via liveness)"]
        C5["Peephole Optimization
        (Instruction fusion)"]
...
| Phase | Description |
| :--- | :--- |
| **CSE** | Common Subexpression Elimination detects identical sub-trees and evaluates them once, storing the result in a temporary register. |
| **Register Allocation** | Analyzes the liveness of intermediate results to reuse register slots, minimizing memory footprint and improving cache locality. |
...
| **Unsafe Hot Path** | Scalar and SIMD loops use `get_unchecked` for register access, relying on compiler-guaranteed bounds safety from the allocation phase. |
