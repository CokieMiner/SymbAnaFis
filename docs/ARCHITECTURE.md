...
flowchart TB
    ...
    subgraph "Bytecode Compiler"
        ...
        (Lowering AST to VIR)"]
        C2["GVN: Global Value Numbering
        (Structural hashing + Normalization)"]
        C3["Division Optimization
        (Redundant Div -> Recip + Mul)"]
        C4["VIR Fusion
        (Pre-scheduling fusion)"]
        C5["Instruction Scheduling
        (Minimize peak register pressure)"]
        C6["Dead Code Elimination
        (Remove unused VIR instructions)"]
        C7["Register Allocation & Peephole
        (Linear Scan & Instruction Fusion)"]
        C8["Return VmEvaluator"]
    end

    subgraph "Instruction Set"
        I1["Arithmetic: Add, Mul, Sub, Div, Pow"]
        I2["Optimized Power: Square, Cube, Pow4, Pow3_2"]
        I3["Fused: MulAdd, MulSub, NegMulAdd, ExpNeg"]
        I4["Inverse: Recip, InvSqrt, InvSquare, InvCube"]
        I5["Math: Sin, Cos, Tan, Exp, Ln, Erf..."]
        I6["Control: LoadConst, Copy, PolyEval"]
    end

    subgraph "Register-Based VM"
        VM1["Pre-allocate stack registers"]
        VM2["Copy params & constants to registers"]
        VM3["Execute instruction sequence"]
        VM4["Unsafe get_unchecked access (fast)"]
        VM5["Return register 0 (result)"]
    end

    E1 --> TW1
    TW1 --> TW2 & TW3 & TW4 & TW5 & TW6 & TW7

    E2 --> C1 --> C2 --> C3 --> C4 --> C5 --> C6 --> C7 --> C8

    C7 -.-> I1 & I2 & I3 & I4 & I5 & I6

    E3 --> VM1 --> VM2 --> VM3 --> VM4 --> VM5
```
...
| Phase | Description |
| :--- | :--- |
| **GVN** | Global Value Numbering detects identical sub-trees and commutative equivalents (a+b vs b+a), evaluating them once and caching the result in a register. |
...
| **Scheduling** | Reorders instructions to minimize the number of simultaneously live registers, reducing the memory footprint of the workspace. |
| **Register Allocation** | Maps virtual registers to physical slots using a linear scan algorithm, ensuring efficient stack usage. |
| **Peephole Fusion** | Scans the generated bytecode for specific patterns to replace them with higher-performance instructions (e.g., `x * y + z` → `MulAdd`). |
| **Unsafe Hot Path** | Scalar and SIMD loops use `get_unchecked` for register access, achieving near-native speed. |

### Peephole Optimization Patterns

| Pattern | Transformation | Benefit |
| :--- | :--- | :--- |
| **FMA** | `Mul(a,b), Add(t,c)` → `MulAdd(a,b,c)` | Higher precision and throughput |
| **Reciprocal** | `Square(x), Recip(t)` → `InvSquare(x)` | Eliminates division stall |
| **Inverse** | `Sqrt(x), Recip(t)` → `InvSqrt(x)` | Uses specialized AVX/scalar fast paths |
| **Power-4** | `Pow4(x), Recip(t)` → `Square(x), InvSquare(t')` | Efficient $x^{-4}$ implementation |
| **Exp-Neg** | `Neg(x), Exp(t)` → `ExpNeg(x)` | Simplified exponential sign handling |
...
| **Per-eval** | ~100-500 ns | ~5-20 ns (up to 40x faster) |
| **Best for** | Single evaluation | Batch evaluation (Monte Carlo, ODEs) |
| **Memory** | O(depth) recursion | Pre-allocated register file |
...
sequenceDiagram
    ...
    loop For each expr
        ...
        Compiler-->>evaluate_parallel: VmEvaluator
    end
...
│   ├── mod.rs               
│   ├── api.rs               # Public Surface: VmEvaluator
│   └── logic/               
│       ├── bytecode/        # Register-based VM logic (v0.8.0 refactor)
│       │   ├── compile/     # Compiler, GVN, Scheduler, RegAlloc
│       │   └── execute/     # Dispatch Loop, SIMD, Parallel drivers
│       └── tree/            # Legacy tree-walking interpreter
│
